...
```{r}
library(rio)
data<-import("https://raw.githubusercontent.com/CahyaniDyahRofiana/PraktikumUTSMPDW/main/Data1.csv")
data
```
...
training_data <- data[1:111,]
testing_data <- data[112:139,]
train_data.ts <- ts(training_data$Harga)
test_data.ts <- ts(testing_data$Harga)
```
...
library(ggplot2)
ggplot() + 
  geom_line(data = training_data, aes(x = No, y = Harga, col = "Data Latih")) +
  geom_line(data = testing_data, aes(x = No, y = Harga, col = "Data Uji")) +
...
```{r}
data.gab<-cbind(aktual=c(train_data.ts,rep(NA,28)),pemulusan=c(data.sma,rep(NA,28)),ramalan=c(data.ramal,rep(data.ramal[length(data.ramal)],27)))
...
```{r}
ts.plot(data.ts, xlab="No", ylab="Harga", main= "SMA N=2 Data Harga")
points(data.ts)
...
error_test.sma = test_data.ts-data.gab[112:139,3]
...
```{r}
#Cara 1 (fungsi ses)
ses.1 <- ses(train_data.ts, h = 28, alpha = 0.2)
plot(ses.1)
ses.1

ses.2<- ses(train_data.ts, h = 28, alpha = 0.7)
plot(ses.2)
ses.2
...
Kasus di atas merupakan contoh inisialisasi nilai parameter $\lambda$ dengan nilai alpha 0,2 dan 0,7 dan banyak periode data yang akan diramalkan adalah sebanyak 28 periode. Selanjutnya akan digunakan fungsi HoltWinters() dengan nilai inisialisasi parameter dan panjang periode peramalan yang sama dengan fungsi ses() .
...
```{r}
#SES
ses.opt <- ses(train_data.ts, h = 28, alpha = NULL)
plot(ses.opt)
ses.opt
...
resid1<-training_data$Harga-ramalan1$fitted
head(resid1)
```
...
resid2<-training_data$Harga-ramalan2$fitted
head(resid2)
...
```{r}
selisih1<-ramalan1$mean-testing_data$Harga
SSEtesting1<-sum(selisih1^2)
MSEtesting1<-SSEtesting1/length(testing_data)

selisih2<-ramalan2$mean-testing_data$Harga
SSEtesting2<-sum(selisih2^2)
MSEtesting2<-SSEtesting2/length(testing_data)

selisihopt<-ramalanopt$mean-testing_data$Harga
